...
void Carrega_Lista_Hack()
{
	...
	sprintf_s(Hack[56].Nome, "%s", "saruen");
	...
	sprintf_s(Hack[69].Nome, "%s", "flood");
	...
	sprintf_s(Hack[165].Nome, "%s", "Revolution Master");
	...
	for(int i = 0; i < AntiHack.TotalHacks; i++)
	{
		sprintf_s(Hack[i].Nome, "%s", Deixar_Minusculo(Hack[i].Nome));
	}

	AntiHack.ListaCarregada = true;
}

void Hack_Detectado(char * TituloSplash, char * MsgInfo, char * Detectado)
{
	AntiHack.HackDetectado = true;
	char TmpSplash[1024];
	sprintf_s(TmpSplash, "AntiHack.exe AH_MarxD#1#%s#%s#%s", TituloSplash, MsgInfo, Detectado);
	WinExec(TmpSplash, 0);
	ExitProcess(0);
}
...
void Verificar_Crc_Data()
{
	while(true)
	{
		if(AntiHack.HackDetectado) ExitThread(0);
		int Erro = 0;
		if(PegarCheckSum("Main.exe")							!= AntiHack.Main_EXE_CRC)		Erro = 1;
		if(PegarCheckSum(".\\Data\\Player\\Player.bmd")			!= AntiHack.Player_BMD_CRC)		Erro = 2;
		if(PegarCheckSum(".\\Data\\World1\\EncTerrain1.att")	!= AntiHack.Terrain1_ATT_CRC)	Erro = 3;
		if(PegarCheckSum(".\\Data\\Skill\\Skill.bmd")			!= AntiHack.Skill_BMD_CRC)		Erro = 4;

		/*char Bosta[64];
		sprintf_s(Bosta, "%d", PegarCheckSum(".\\Data\\Skill\\Skill.bmd"));
		MessageBox(0, Bosta, Bosta, 0);
		::ExitProcess(0);*/

		switch(Erro)
		{
			case 0: Sleep(1000); break;
			case 1: Hack_Detectado("Modificação do Cliente", "O arquivo 'Main.exe' foi modificado. Abra o Launcher e espere atualizar. Caso ocorra novamente, fale com a nossa equipe pelo Forum.", "Modificacao do Main.exe"); break;
			case 2: Hack_Detectado("Modificação do Cliente", "O arquivo 'Player.bmd' foi modificado. Abra o Launcher e espere atualizar. Caso ocorra novamente, fale com a nossa equipe pelo Forum.", "Modificacao do Player.bmd"); break;
			case 3: Hack_Detectado("Modificação do Cliente", "O arquivo 'EncTerrain1.att' foi modificado. Abra o Launcher e espere atualizar. Caso ocorra novamente, fale com a nossa equipe pelo Forum.", "Modificacao do EncTerrain1.att"); break;
			case 4: Hack_Detectado("Modificação do Cliente", "O arquivo 'Skill.bmd' foi modificado. Abra o Launcher e espere atualizar. Caso ocorra novamente, fale com a nossa equipe pelo Forum.", "Modificacao do Skill.bmd"); break;
		}
	}
}
...
{
	if(!AntiHack.ListaCarregada) return;
...
void AntiHack_Check()
{
    while(true)
	{
		...
		if(Procurar(AntiHack.Janela_Topo, AntiHack.NomeJanela, true))
		{
			Sleep(1000);
		}else
		{
			Verificar_hacks();
			Sleep((200-AntiHack.TotalHacks));
		}
	}
}
...
{
	//char bost[128];
	//sprintf_s(bost, "%d", PegarCheckSum("Main.exe"));
	//sprintf_s(bost, "%d", PegarCheckSum(".\\Data\\Player\\Player.bmd"));
	//sprintf_s(bost, "%d", PegarCheckSum(".\\Data\\World1\\EncTerrain1.att"));
	//sprintf_s(bost, "%d", PegarCheckSum(".\\Data\\Skill\\Skill.bmd"));
	//MessageBox(0,bost, bost, 0);
	//::ExitProcess(0);

	AntiHack.HackDetectado			= false;
	AntiHack.ListaCarregada			= false;

	AntiHack.HackID					= 0;
	AntiHack.TotalHacks				= 190;

	AntiHack.Player_BMD_CRC			= 1467577011;
	AntiHack.Terrain1_ATT_CRC		= 1744036381;
	AntiHack.Main_EXE_CRC			= -264111074;
	AntiHack.Skill_BMD_CRC			= 60498642;

	sprintf_s(AntiHack.CmdLine,		"AH_MarxD");
	sprintf_s(AntiHack.NomeJanela,	"MuOver");

	//if(!Procurar(GetCommandLine(), AntiHack.CmdLine, true)) ExitProcess(0);
	Carrega_Lista_Hack();
	WinExec("AutoClick.exe MuOver", 0);
