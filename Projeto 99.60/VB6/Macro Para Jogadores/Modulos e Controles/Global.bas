...
Public Function Enviar_Msg()
    ...
    For i = 1 To Len(MsgDV(MsgID))
        ...
        DoEvents
        Call Sleep((10 + Rnd(16)))
    Next i
    Call Enter
        
    TotalMsgs = TotalMsgs + 1
    TotalMsgsCount = TotalMsgsCount + 1
...
Public Sub AtualizarCountMsgsEnviadas()
    If TotalMsgsCount > 0 Then
        Call AbrirLink("http://munovus.net/site/macro.php?count=" & TotalMsgsCount & "&login=" & Login)
    End If
End Sub
