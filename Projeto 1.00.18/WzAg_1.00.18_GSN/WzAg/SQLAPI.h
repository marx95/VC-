...
#define SQLAPI_WIN32
#ifdef _MSC_VER
	#pragma warning(disable : 4290 4121)
#endif
#endif
...
/*!
 * \addtogroup typedefs SQLAPI++ defined types and enums
 * \{
 */
typedef
enum
...
typedef bool (*PreHandleException_t)
	(SAException &x);
...
{
	void InitZero();
	void setFromPlainString(const SAChar *sVal);
	void setFromExpString(const SAString &sVal);
...
public:
	...
	static SADateTime currentDateTimeWithFraction();
...
public:
	...
	void Connect(
		...
		SAClient_t eSAClient = SA_Client_NotSpecified) SQLAPI_THROW(SAException);
